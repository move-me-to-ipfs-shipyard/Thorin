              ...
              (let [txn-idBA  (:t msg)
                ...
                (if (or (not txn-idBA) (not= (bytes.runtime.core/alength node-idBA) 20))
                  ...
                  (put! send|
                        {:msg  {:t txn-idBA
                                :y "r"
                                :r {:id self-idBA #_(find.seed/gen-neighbor-id node-idB (:self-idBA @stateA))}}
                         :host host
                         :port port})))
                ...
                (if (or (not txn-idBA) (not= (bytes.runtime.core/alength node-idBA) 20))
                  ...
                  (put! send|
                        {:msg {:t txn-idBA
                               :y "r"
                               :r {:id self-idBA #_(find.seed/gen-neighbor-id node-idB (:self-idBA @stateA))
                                   :nodes (find.seed/encode-nodes (take 8 (:routing-table @stateA)))}}
                         :host host
                         :port port})))
                ...
                (if (or (not txn-idBA) (not= (bytes.runtime.core/alength node-idBA) 20) (not= (bytes.runtime.core/alength infohashBA) 20))
                  ...
                  (do
                    ...
                    (put! send|
                          {:msg {:t txn-idBA
                                 :y "r"
                                 :r {:id self-idBA #_(find.seed/gen-neighbor-id infohashBA (:self-idBA @stateA))
                                     :nodes (find.seed/encode-nodes (take 8 (:routing-table @stateA)))
                                     :token tokenBA}}
                           :host host
                           :port port}))))
...
(defn valid-for-ping?
  ...
  (or
   (not (:pinged-at node))
   (> (- (find.seed/now) (:pinged-at node)) (* 2 60 1000))))
...
(defn process-routing-table
  ...
  (let [routing-table-comparator (find.seed/hash-key-distance-comparator-fn self-idBA)
        stop| (chan 1)]
           ...
           (let [state @stateA]
             (doseq [[id node] (->>
               ...
               (take! (send-krpc-request
                      ...
                      (fn [value]
                        (if value
                          (swap! stateA update-in [:routing-table id] assoc :pinged-at (find.seed/now))
        ...
        (when-let [nodes (<! dht-keyspace-nodes|)]
          (let [dht-keyspace-keys (keys (:dht-keyspace @stateA))]
            (doseq [node nodes]
              (let [closest-key (->>
                                 dht-keyspace-keys
                                 (sort-by identity (find.seed/hash-key-distance-comparator-fn (:idBA node)))
                                 first)]
           ...
           (let [state @stateA]
             (doseq [[k routing-table] (:dht-keyspace state)
               ...
               (take! (send-krpc-request
                      ...
                      (fn [value]
                        (if value
                          (swap! stateA update-in [:dht-keyspace k id] assoc :pinged-at (find.seed/now))
