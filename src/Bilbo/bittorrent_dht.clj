          ...
          (let [msg-y (some-> (:y msg) (Bilbo.bytes/to-string))
            ...
            (cond
              ...
              (let [txn-idBA  (:t msg)
                ...
                (if (or (not txn-idBA) (not= (Bilbo.bytes/alength node-idBA) 20))
                  ...
                  (put! send|
                        {:msg  {:t txn-idBA
                                :y "r"
                                :r {:id self-idBA #_(Bilbo.seed/gen-neighbor-id node-idB (:self-idBA @stateA))}}
                         :host host
                         :port port})))
                ...
                (if (or (not txn-idBA) (not= (Bilbo.bytes/alength node-idBA) 20))
                  ...
                  (put! send|
                        {:msg {:t txn-idBA
                               :y "r"
                               :r {:id self-idBA #_(Bilbo.seed/gen-neighbor-id node-idB (:self-idBA @stateA))
                                   :nodes (Bilbo.seed/encode-nodes (take 8 (:routing-table @stateA)))}}
                         :host host
                         :port port})))
              ...
              (let [infohashBA (get-in msg [:a :info_hash])
                    txn-idBA (:t msg)
                    node-idBA (get-in msg [:a :id])
                    tokenBA (Bilbo.bytes/copy-byte-array infohashBA 0 4)]
                (if (or (not txn-idBA) (not= (Bilbo.bytes/alength node-idBA) 20) (not= (Bilbo.bytes/alength infohashBA) 20))
                  ...
                  (do
                    ...
                    (put! send|
                          {:msg {:t txn-idBA
                                 :y "r"
                                 :r {:id self-idBA #_(Bilbo.seed/gen-neighbor-id infohashBA (:self-idBA @stateA))
                                     :nodes (Bilbo.seed/encode-nodes (take 8 (:routing-table @stateA)))
                                     :token tokenBA}}
                           :host host
                           :port port}))))
...
(defn valid-for-ping?
  ...
  (or
   (not (:pinged-at node))
   (> (- (Bilbo.seed/now) (:pinged-at node)) (* 2 60 1000))))
...
(defn process-routing-table
  ...
  (let [routing-table-comparator (Bilbo.seed/hash-key-distance-comparator-fn self-idBA)
        stop| (chan 1)]
          ...
          (let [routing-table (:routing-table @stateA)]
            ...
            (if (and (>= i 4) (> time-total 4000))
              (let [nodes (:routing-table @stateA)
                ...
                (recur n 0 (Bilbo.seed/now) 0))
              (recur n (inc i) (Bilbo.seed/now) (+ time-total (- (Bilbo.seed/now) ts)))))))
      (close! stop|))
           ...
           (let [state @stateA]
             (doseq [[id node] (->>
               ...
               (take! (send-krpc-request
                      ...
                      (fn [value]
                        (if value
                          (swap! stateA update-in [:routing-table id] assoc :pinged-at (Bilbo.seed/now))
...
(defn process-dht-keyspace
  ...
  (let [routing-table-comparator (Bilbo.seed/hash-key-distance-comparator-fn self-idBA)
    ...
    (doseq [[id routing-table] (:dht-keyspace @stateA)]
      (swap! stateA update-in [:dht-keyspace id] (partial into (sorted-map-by (Bilbo.seed/hash-key-distance-comparator-fn (Bilbo.codec/hex-to-bytes id))))))
        ...
        (when-let [nodes (<! dht-keyspace-nodes|)]
          (let [dht-keyspace-keys (keys (:dht-keyspace @stateA))]
            (doseq [node nodes]
              (let [closest-key (->>
                                 dht-keyspace-keys
                                 (sort-by identity (Bilbo.seed/hash-key-distance-comparator-fn (:idBA node)))
                                 first)]
                         ...
                         (->>
                          ...
                          (map (fn [[id routing-table]]
                                 [id (->> routing-table
                                          ...
                                          (into (sorted-map-by (Bilbo.seed/hash-key-distance-comparator-fn (Bilbo.codec/hex-to-bytes id)))))]))
                          (into (sorted-map)))))
                (recur n 0 (Bilbo.seed/now) 0))
              (recur n (inc i) (Bilbo.seed/now) (+ time-total (- (Bilbo.seed/now) ts)))))))
      (close! stop|))
           ...
           (let [state @stateA]
             (doseq [[k routing-table] (:dht-keyspace state)
               ...
               (take! (send-krpc-request
                      ...
                      (fn [value]
                        (if value
                          (swap! stateA update-in [:dht-keyspace k id] assoc :pinged-at (Bilbo.seed/now))
